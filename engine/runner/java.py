...
class JavaRunner(BaseRunner):
    """
    Запуск решений на Java.
    Ожидается, что имя класса совпадает с Solution.
    """
    def run(self, solution_path, input_data, time_limit, memory_limit):
        temp_dir = tempfile.mkdtemp()
        try:
            # Копируем файл решения во временную папку
            ...
            # Запуск (ожидается класс Solution с методом main)
